...
- Native build gates use a fresh output directory, require exit code zero and the CLI JSON's exact output path, and verify macOS executable architectures from `CFBundleExecutable`. Old bundles, empty directories, or timeout artifacts never prove a successful build.
...
- Stable `V*` Docker publication updates both the version tag and `latest`; checking only the version tag does not prove the documented default Docker command receives the new image. Quality validates the actual image CLI and both DEB/AppImage artifacts before a release tag is pushed.
