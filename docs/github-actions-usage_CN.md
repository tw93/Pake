...
介绍、目标目录、下载、进度和完成页面均由 QtIFW 提供，具有各平台的原生风格，
并可通过仓库中的控制脚本和组件脚本继续扩展。

- Windows：`online_windows_format` 可选择应用专属 `.msi` 或 `.exe`。MSI
  安装并打开 QtIFW 在线向导；EXE 是对同一个在线 MSI 的完全无窗口包装。
  `online_exe_icon` 同时控制包装器和安装器图标。载体版本固定为 `255.0.0`，
  下载的应用仍使用 Pake 最新正式 Release 版本。
- macOS：`.dmg` 中包含原生 QtIFW 安装器 `.app`，下载最新应用包并安装到
  `/Applications`。
- Linux：`.AppImage` 中包含 QtIFW 安装器，下载最新应用 AppImage 并进行常规
  用户级安装。
...
更新。QtIFW 会验证仓库元数据与归档摘要，并直接在安装页面显示下载进度。滚动
Release 继续只保留当前和上一个成功组件归档及 manifest。
...
- 配置保存在 `pake-online-config` 分支；QtIFW 仓库使用独立的
  ...
  push 中占用一个 runner。
- Windows 应用按用户安装；macOS 写入 `/Applications` 时可能请求管理员授权；
  Linux 默认使用用户级目标目录。
